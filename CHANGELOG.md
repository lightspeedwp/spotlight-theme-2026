...
- `patterns/fair-badge.php` — the Press Council/FAIR certification image, registered as a pattern (rather than inlined in `parts/footer.html`) so its `<img src>` can go through `get_theme_file_uri()` instead of a hard-coded theme path.
