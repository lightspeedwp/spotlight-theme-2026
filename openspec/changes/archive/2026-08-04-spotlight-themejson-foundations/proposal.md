...
- **Affected files**: `theme.json` (settings.color.palette, settings.typography, settings.spacing, settings.border, settings.custom, settings.layout, styles.elements). `assets/fonts/` gains Source Sans 3 and Libre Baskerville font files (self-hosted, matching this repo's existing font-file convention).
