...
- **The Press Council/FAIR certification badge** — now a real `wp:image` block, registered as `patterns/fair-badge.php` and referenced from `parts/footer.html` via `<!-- wp:pattern {"slug":"spotlight-theme-2026/fair-badge"} /-->`. Patterns are PHP, unlike template parts, so its `<img src>` goes through `esc_url( get_theme_file_uri( 'assets/logos/fair-logo.png' ) )` instead of a hard-coded `/wp-content/themes/spotlight-theme-2026/...` path — portable across custom `WP_CONTENT_URL` setups, multisite, or a re-slugged copy of this starter. The prior `background-image`-on-an-empty-`<p>` hack has been removed. Same for the "10 years of impact" item's numeral — no longer a text substitute needing its own sizing rule, it's a real inline SVG in the same 60×60 box as the other four icons (an SVG `<text>` element set in the heading typeface).
