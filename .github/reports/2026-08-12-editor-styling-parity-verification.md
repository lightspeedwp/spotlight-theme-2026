...
Every `style="..."` attribute *authored by this theme* in `templates/*.html` and `parts/*.html` resolves to a `var(--wp--preset--...)` custom property (spacing, colour, font-size), not a literal value. Custom properties are emitted from the same `theme.json` in both the front end and the editor, so these render identically in both places.

This excludes markup generated by the Icon Block plugin (`outermost/icon-block`), which does contain literal values — a `width:20px`-style inline style on its `.icon-container` wrapper and `fill="black"`/`fill="#F5F4F8"` on the SVG path — plus the ordinary block-level `width` attribute on `wp:site-logo`/`outermost/icon-block` (e.g. `{"width":140}`, `{"width":20}`). None of this is theme-authored styling to keep token-driven: it's serialised once into saved block markup at save time (see "Icon rendering depends on the Icon Block plugin" below), identical in both renderers, so it carries no parity risk even though it isn't token-based.
...
- **Icon rendering depends on the Icon Block plugin, not theme CSS.** `outermost/icon-block` serialises a full inline SVG (with literal `fill` attributes, not preset colours) plus a literal `width` inline style on its `.icon-container` wrapper, at save time — so there is nothing for the theme to keep in sync — both renderers show the same saved markup. This is the source of the literal (non-token) values noted as excluded from finding 1's "all authored styling is token-driven" claim. Documented here because it's a real editor-availability dependency (the plugin must be active in both environments), even though it's not a styling risk.
